...
@ServerEndpoint("/serverC")
public class Server {
  private World world = null;
  private Entity player = null;
  private Input input = null;
  private int worldX = 2000;
  private int worldY = 2000;
  @OnOpen public void onOpen(Session ses){
    ...
    input = new Input();
    player = new Entity(ses,input,new SpaceshipUpdate(worldX,worldY),new SpaceshipReport())
        .setX(0).setY(0).setVelX(0).setVelY(0).setAngle(0).setColor("silver").setType("ship").setRadius(20);
  ...
  @OnMessage public void onMessage(String mes, Session ses){
    ...
    player.setAngle(Double.parseDouble(mes.substring(6)));
